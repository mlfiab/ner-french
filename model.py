...
"""
import os
os.environ['TRANSFORMERS_CACHE'] = '/Home/Users/ytchouka/WORK/tmp/cache/'
from adatasets import get_conll_data
from network import NERDANetwork, FLAUNERNetwork
from prediction import predict, predict_text
...
class FLAUNER:
    ...
    def __init__(self,
                 ...
                 max_len: int = 128,
                 network: torch.nn.Module = FLAUNERNetwork,
                 dropout: float = 0.1,
        ...
        if tokenizer_parameters is None:
        ...
        if transformer == 'Jean-Baptiste/camembert-ner-with-dates':
            self.transformer_model = AutoModelForTokenClassification.from_pretrained(transformer)
        else:
        ...
        self.network = FLAUNERNetwork(self.transformer_model, self.device, len(tag_complete), dropout=dropout)
    ...
    def evaluate_performance(self, dataset: dict, **kwargs) -> pd.DataFrame:
        ...
        f1_micro = pd.DataFrame({'Level': ['AVG_MICRO'],
                                  ...
                                  'Precision': np.nan,
                                  'Recall': np.nan})
        #f1_micro_ = pd.DataFrame({'Level': ['AVG_MICRO'], 'F1-Score': [f1_micro[2]]})
        #df = df.append(f1_mic)
